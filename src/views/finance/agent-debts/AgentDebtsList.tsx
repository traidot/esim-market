...
const AgentDebtsList = () => {
  const agents = [
    { id: 'a001', name: 'TravelConnect Solutions', code: 'TR', tier: 'PLATINUM', tierColor: 'primary', debt: 5240.00, creditLimit: 50000.00, status: 'Active' },
    { id: 'a002', name: 'Global eSIM Hub', code: 'GL', tier: 'GOLD', tierColor: 'warning', debt: 8120.50, creditLimit: 10000.00, status: 'Warning' },
    { id: 'a003', name: 'CheapData Agency', code: 'CH', tier: 'SILVER', tierColor: 'secondary', debt: 15.00, creditLimit: 100.00, status: 'Active' },
    { id: 'a004', name: 'Nomad Partner', code: 'NO', tier: 'GOLD', tierColor: 'warning', debt: 0.00, creditLimit: 10000.00, status: 'Active' },
    { id: 'a005', name: 'Asia Roaming', code: 'AS', tier: 'PLATINUM', tierColor: 'primary', debt: 45000.00, creditLimit: 50000.00, status: 'Critical' },
  ]

  // Mock global metrics
  const totalDebt = agents.reduce((acc, curr) => acc + curr.debt, 0)
  const totalCreditLimit = agents.reduce((acc, curr) => acc + curr.creditLimit, 0)
  const totalAgents = agents.length
  const criticalAgents = agents.filter(a => (a.debt / a.creditLimit) > 0.8).length
      ...
      <Grid2 container spacing={6} className='mbe-6'>
        ...
        <Grid2 size={{ xs: 12, md: 3 }}>
          <Card className='border-none shadow-sm'>
            <CardContent className='p-6'>
              <Typography variant='caption' className='font-bold text-slate-500 uppercase'>Tổng Hạn Mức Đã Cấp</Typography>
              <Typography variant='h3' className='font-black'>
                {totalCreditLimit.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
              </Typography>
            </CardContent>
          </Card>
        </Grid2>
        <Grid2 size={{ xs: 12, md: 3 }}>
          <Card className='border-none shadow-sm'>
            <CardContent className='p-6'>
              <Typography variant='caption' className='font-bold text-slate-500 uppercase'>Số Đại lý đang Nợ</Typography>
              <Typography variant='h3' className='font-black text-primary'>
                {agents.filter(a => a.debt > 0).length} <span className='text-sm text-slate-400'>/ {totalAgents}</span>
              </Typography>
            </CardContent>
          </Card>
        </Grid2>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell className='font-black uppercase text-[11px]'>Cấp bậc</TableCell>
                <TableCell className='font-black uppercase text-[11px] text-right'>Công Nợ Hiện Tại</TableCell>
            ...
            <TableBody>
              {agents.map((agent) => {
                const usageRatio = (agent.debt / agent.creditLimit) * 100
                ...
                return (
                  <TableRow key={agent.id} hover>
                    ...
                    <TableCell>
                      <Chip label={agent.tier} color={agent.tierColor as any} size='small' variant='tonal' className='font-bold' />
                    </TableCell>
                    <TableCell className='text-right'>
                      <Typography variant='subtitle2' className={`font-black ${usageRatio > 80 ? 'text-error' : ''}`}>
                        {agent.debt.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                      </Typography>
                    </TableCell>
                    <TableCell>

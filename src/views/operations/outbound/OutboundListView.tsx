'use client'

import { useState, useMemo } from 'react'
import Card from '@mui/material/Card'
import CardContent from '@mui/material/CardContent'
import Typography from '@mui/material/Typography'
import Chip from '@mui/material/Chip'
import TextField from '@mui/material/TextField'
import Button from '@mui/material/Button'
import IconButton from '@mui/material/IconButton'

import Link from 'next/link'
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
  useReactTable,
  getPaginationRowModel
} from '@tanstack/react-table'

import PageHeader from '@/components/layout/shared/PageHeader'
import TablePaginationComponent from '@/components/TablePaginationComponent'
import { outboundData } from '@/data/operationsData'
import AddOutboundDialog from './AddOutboundDialog'
import { fuzzyFilter } from '@/utils/tableUtils'

const columnHelper = createColumnHelper<any>()

const OutboundListView = () => {
  const [data] = useState(() => [...outboundData])
  const [isDialogOpen, setIsDialogOpen] = useState(false)

  const columns = useMemo(
    () => [
      columnHelper.accessor('id', {
        header: 'Order ID',
        cell: info => (
          <Typography 
            color='primary' 
            className='font-bold' 
            component={Link} 
            href={`/3m/operations/outbound/${info.getValue()}`}
            
          >
            {info.getValue()}
          </Typography>
        )
      }),
      columnHelper.accessor('customer', {
        header: 'Customer',
        cell: info => <Typography color='text.primary'>{info.getValue()}</Typography>
      }),
      columnHelper.accessor('requestDate', {
        header: 'Order Date',
        cell: info => <Typography variant='body2'>{info.getValue()}</Typography>
      }),
      columnHelper.accessor('items', {
        header: 'Items',
        cell: info => <Typography variant='body2'>{info.getValue()}</Typography>
      }),
      columnHelper.accessor('status', {
        header: 'Status',
        cell: info => {
          const val = info.getValue()
          let color: any = 'primary'
          if (val === 'Pending') color = 'warning'
          if (val === 'Picking') color = 'info'
          if (val === 'Shipped') color = 'success'
          return <Chip label={val} color={color} size='small' variant='tonal' />
        }
      }),
      columnHelper.accessor('id', {
         id: 'actions',
         header: 'Actions',
         cell: info => (
           <div className='flex gap-2'>
             <Button 
               size='small' 
               variant='tonal'
               component={Link}
               href={`/3m/operations/outbound/picking/${info.row.original.id}`}
               startIcon={<i className='tabler-hand-finger' />}
             >
               Pick
             </Button>
             <IconButton 
               size='small' 
               color='primary'
               component={Link}
               href={`/3m/operations/outbound/${info.row.original.id}`}
             >
               <i className='tabler-eye text-[22px]' />
             </IconButton>
           </div>
         )
      })
    ],
    []
  )

  const table = useReactTable({
    data,
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
    getCoreRowModel: getCoreRowModel(),
    getPaginationRowModel: getPaginationRowModel(),
  })

  return (
    <>
      <PageHeader
        title='Outbound Management'
        description='Issue goods and manage customer deliveries'
        breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Operations' }, { label: 'Outbound' }]}
      />

      <Card className='mt-6'>
        <CardContent className='flex items-center justify-between'>
          <TextField
            size='small'
            placeholder='Search Orders...'
            sx={{ width: 250 }}
          />
          <Button variant='contained' color='secondary' startIcon={<i className='tabler-package-export' />} onClick={() => setIsDialogOpen(true)}>
            Create Issue
          </Button>
        </CardContent>
        <div className='overflow-x-auto'>
          <table className='w-full'>
            <thead className='border-be'>
              {table.getHeaderGroups().map(headerGroup => (
                <tr key={headerGroup.id}>
                  {headerGroup.headers.map(header => (
                    <th key={header.id} className='text-start p-4 bg-slate-50 uppercase text-xs font-semibold'>
                      {header.isPlaceholder ? null : flexRender(header.column.columnDef.header, header.getContext())}
                    </th>
                  ))}
                </tr>
              ))}
            </thead>
            <tbody>
              {table.getRowModel().rows.map(row => (
                <tr key={row.id} className='border-be hover:bg-slate-50'>
                  {row.getVisibleCells().map(cell => (
                    <td key={cell.id} className='p-4'>
                      {flexRender(cell.column.columnDef.cell, cell.getContext())}
                    </td>
                  ))}
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <TablePaginationComponent table={table} />
      </Card>

      <AddOutboundDialog open={isDialogOpen} onClose={() => setIsDialogOpen(false)} />
    </>
  )
}

export default OutboundListView

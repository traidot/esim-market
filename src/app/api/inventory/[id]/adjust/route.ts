...
import { InventoryAdjustSchema } from '@/server/schemas/outbound.schema'
...
export const POST = withApi(async (req: NextRequest, ctx: RouteContext) => {
  const auth = requirePermission('inventory.update', req)
  const { id } = await ctx.params
  const body = await readJson(req)
  const parsed = v.safeParse(InventoryAdjustSchema, body)
  if (!parsed.success) {
    throw ApiErrors.validation('Dữ liệu không hợp lệ', { issues: parsed.issues })
  }
  return ok(await adjustInventory(id, { ...parsed.output, byUserId: auth.userId }))
})

...
import { created, withApi } from '@/server/lib/api-response'
import { requirePermission } from '@/server/lib/auth-context'
import { ContainerLoadItemCreateSchema } from '@/server/schemas/container.schema'
import { addLoadItem } from '@/server/services/container.service'
...
export const POST = withApi(async (req: NextRequest, ctx: RouteContext) => {
  ...
  const parsed = v.safeParse(ContainerLoadItemCreateSchema, body)
  if (!parsed.success) {
    throw ApiErrors.validation('Dữ liệu không hợp lệ', { issues: parsed.issues })
  }
  const item = await addLoadItem(id, parsed.output)
  return created(item)
})

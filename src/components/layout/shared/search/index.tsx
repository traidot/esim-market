  ...
  return (
    <div cmdk-footer=''>
      ...
      <div className='flex items-center gap-1'>
        ...
        <span>{t('header.searchOpen')}</span>
      </div>
      <div className='flex items-center gap-1'>
        <kbd>esc</kbd>
        <span>{t('header.searchClose')}</span>
      </div>
    </div>
  )
}
    ...
    <>
      {isBreakpointReached || settings.layout === 'horizontal' ? (
        ...
        <div className='flex items-center gap-2 cursor-pointer' onClick={() => setOpen(true)}>
          ...
          <div className='whitespace-nowrap select-none text-textDisabled'>{t('header.search')} ⌘K</div>
        </div>
      )}
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <CommandDialog open={open} onOpenChange={setOpen} label={t('header.search')}>
